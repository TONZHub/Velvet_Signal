...
function benchmarkPatch({ patchId, publishedAt, claimId, statement, supersedes = [] }) {
  return { patch_id: patchId, desk: "Maker Edition", title: "Synthetic supersession fixture", scope: "VS-Bench synthetic fixture", published_at: publishedAt, valid_until: "2027-08-28", delivery: { status: "delivered", approved: true }, claims: [{ id: claimId, statement, status: "verified", source_ids: ["BENCH-SRC-1"], ...(supersedes.length ? { supersedes } : {}) }], sources: [{ id: "BENCH-SRC-1", publisher: "VS-Bench", url: "https://example.test/fixture" }] };
}
